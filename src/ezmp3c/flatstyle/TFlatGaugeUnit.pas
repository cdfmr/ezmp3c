...
uses
  ...
  Forms, StdCtrls, ExtCtrls, Consts, FlatUtilitys, DIALOGS;
...
var
  R: TRect;
  MemBmpLeft, MemBmpRight: TBitmap;
  XProgress: Integer;
  PercentText: string;
  TX, TY: Integer;
begin
  try
    PercentText := IntToStr((FProgress - FMinValue) * 100 div (FMaxValue - FMinValue)) + ' %';
  except
    PercentText := 'error';
  end;

  Canvas.Font.Assign(Self.Font);
  TX := (Width - Canvas.TextWidth(PercentText)) div 2;
  TY := (Height - Canvas.TextHeight(PercentText)) div 2;

  MemBmpLeft := TBitmap.Create;
  try
    MemBmpLeft.Width := Width;
    MemBmpLeft.Height := Height;
    with MemBmpLeft.Canvas do
    begin
      ...
      begin
        ...
        Font.Color := Color;
        Brush.Style := bsClear;
        TextOut(TX, TY, PercentText);
      end;

      MemBmpRight := TBitmap.Create;
      try
        MemBmpRight.Width := Width;
        MemBmpRight.Height := Height;
        with MemBmpRight.Canvas do
        begin
          Pen.Color := FBorderColor;
          Brush.Color := Color;
          Rectangle(ClientRect);
        ...
        InflateRect(R, -3, -3);
        try
          XProgress := R.Left + (FProgress - FMinValue) * (R.Right - R.Left) div (FMaxValue - FMinValue);
        except
          XProgress := 0;
        end;
        R := ClientRect;
        R.Left := XProgress;
        MemBmpLeft.Canvas.CopyRect(R, MemBmpRight.Canvas, R);
      finally
        MemBmpRight.Free;
      end;
    end;
    Canvas.CopyRect(ClientRect, MemBmpLeft.Canvas, ClientRect);
  finally
    MemBmpLeft.Free;
  end;
end;
